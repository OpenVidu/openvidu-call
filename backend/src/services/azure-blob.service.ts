...
import { CALL_AZURE_ACCOUNT_NAME, CALL_AZURE_ACCOUNT_KEY, CALL_AZURE_CONTAINER_NAME } from '../config.js';
...
export class AzureBlobService {
	...
	private constructor() {
		if (!CALL_AZURE_ACCOUNT_NAME || !CALL_AZURE_ACCOUNT_KEY || !CALL_AZURE_CONTAINER_NAME) {
			throw new Error('Azure Blob Storage configuration is incomplete');
		}

		const AZURE_STORAGE_CONNECTION_STRING = `DefaultEndpointsProtocol=https;AccountName=${CALL_AZURE_ACCOUNT_NAME};AccountKey=${CALL_AZURE_ACCOUNT_KEY};EndpointSuffix=core.windows.net`;
		this.blobServiceClient = BlobServiceClient.fromConnectionString(AZURE_STORAGE_CONNECTION_STRING);
		this.containerClient = this.blobServiceClient.getContainerClient(CALL_AZURE_CONTAINER_NAME);
	}
